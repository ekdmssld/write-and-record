...
P2 Social (일부 구현됨):
- QR 초대 코드 (구현됨) — 초대 시트에서 초대 URL을 QR 이미지로 보여주고 공유할 수 있다. CoreImage만 사용, 외부 SDK 없음. 스캔해서 자동으로 친구가 되는 흐름(딥링크/유니버설 링크 수신)은 여전히 없다 — 초대 링크 실동작은 `docs/13` 백로그의 별도 항목.
